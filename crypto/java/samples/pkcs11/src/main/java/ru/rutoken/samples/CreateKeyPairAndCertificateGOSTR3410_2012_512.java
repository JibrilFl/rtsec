...
import static ru.rutoken.pkcs11jna.Pkcs11Tc26Constants.CKK_GOSTR3410_512;
import static ru.rutoken.pkcs11jna.Pkcs11Tc26Constants.CKM_GOSTR3410_512_KEY_PAIR_GEN;
import static ru.rutoken.samples.pkcs11utils.Constants.ATTR_CRYPTO_PRO_A_GOSTR3410_2012_512;
import static ru.rutoken.samples.pkcs11utils.Constants.ATTR_GOSTR3411_2012_512;
import static ru.rutoken.samples.utils.CommonUtil.*;
import static ru.rutoken.samples.utils.Util.*;

/**
 * Sample of generating GOST R 34.10-2012 (512 bits) key pair and importing corresponding certificate.
 */
public class CreateKeyPairAndCertificateGOSTR3410_2012_512 {
    /**
     ...
     */
    public static byte[] KEY_PAIR_ID = "Sample GOST R 34.10-2012 (512 bits) key pair (Aktiv Co.)".getBytes();

    static {
        publicKeyTemplate = (CK_ATTRIBUTE[]) (new CK_ATTRIBUTE()).toArray(8);
        publicKeyTemplate[0].setAttr(CKA_CLASS, CKO_PUBLIC_KEY);
        publicKeyTemplate[1].setAttr(CKA_LABEL, "Sample GOST R 34.10-2012 (512 bits) Public Key (Aktiv Co.)");
        publicKeyTemplate[2].setAttr(CKA_ID, KEY_PAIR_ID);
        publicKeyTemplate[3].setAttr(CKA_KEY_TYPE, CKK_GOSTR3410_512); // Key type - GOST R 34.10-2012 (512 bits)
        ...
        publicKeyTemplate[6].setAttr(CKA_GOSTR3410_PARAMS,
                ATTR_CRYPTO_PRO_A_GOSTR3410_2012_512); // Parameters of GOST R 34.10-2012 (512 bits) algorithm
        publicKeyTemplate[7].setAttr(CKA_GOSTR3411_PARAMS,
                ATTR_GOSTR3411_2012_512); // Parameters of GOST R 34.11-2012 (512) algorithm

        privateKeyTemplate = (CK_ATTRIBUTE[]) (new CK_ATTRIBUTE()).toArray(11);
        privateKeyTemplate[0].setAttr(CKA_CLASS, CKO_PRIVATE_KEY);
        privateKeyTemplate[1].setAttr(CKA_LABEL, "Sample GOST R 34.10-2012 (512 bits) Private Key (Aktiv Co.)");
        privateKeyTemplate[2].setAttr(CKA_ID, KEY_PAIR_ID);
        privateKeyTemplate[3].setAttr(CKA_KEY_TYPE, CKK_GOSTR3410_512); // Key type - GOST R 34.10-2012 (512 bits)
        ...
        privateKeyTemplate[7].setAttr(CKA_GOSTR3410_PARAMS,
                ATTR_CRYPTO_PRO_A_GOSTR3410_2012_512); // Parameters of GOST R 34.10-2012 (512 bits) algorithm
        privateKeyTemplate[8].setAttr(CKA_GOSTR3411_PARAMS,
                ATTR_GOSTR3411_2012_512); // Parameters of GOST R 34.11-2012 (512) algorithm
        ...
        privateKeyTemplate[9].setAttr(CKA_START_DATE, startDate.getPointer(), startDate.size());
        privateKeyTemplate[10].setAttr(CKA_END_DATE, endDate.getPointer(), endDate.size());
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            NativeLong rv = pkcs11.C_GenerateKeyPair(session,
                    new CK_MECHANISM(CKM_GOSTR3410_512_KEY_PAIR_GEN, null, 0),
